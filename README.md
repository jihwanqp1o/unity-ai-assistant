...
`core/rag.py`, `core/prompt_builder.py`는 변경 없이 그대로 백엔드에서 재사용되므로,
아래 "이 세션에서 한 일" / "RAG 정확도 이슈" 기록은 여전히 유효하다. (LLM 호출 모듈은
이후 2026-07-22에 `core/claude_client.py`(Anthropic)에서 `core/llm_client.py`(Gemini,
무료 티어)로 교체됐다 — 아래 "LLM 제공자" 섹션 참고.)
...
- **LLM API 실제 호출(real 모드, Vision 분석 포함)** — 현재는 Gemini(`core/llm_client.py`)로
  바뀌었고 `GEMINI_API_KEY`가 아직 없어 mock 모드로만 검증된 상태. 무료 키 발급
  (https://aistudio.google.com/apikey) 후 `setx GEMINI_API_KEY "..."`로 등록하고
  `scripts/run_scenario_eval.py`를 real 모드로 재실행해 실제 LLM 코드 제안 품질을 확인해야
  한다 (Day 6, 아래 표 참조).
...
docker run -p 8000:8000 `
  ...
  -e DATABASE_URL=postgresql://<영속 DB 연결 문자열> `
  unity-ai-assistant
```

- 특정 PaaS(Render/Fly.io/Railway 등)는 지정하지 않았다 — 이 Dockerfile을 그대로 올리면 된다.
- `DATABASE_URL`을 지정하지 않으면 컨테이너 내 SQLite 파일(`backend/data/app.db`)로
  떨어지는데, 컨테이너 재생성 시 이 파일은 사라진다 — 반드시 영속 Postgres(Render 배포는
  `render.yaml`이 자동 프로비저닝)를 붙여야 한다. 실제로 이걸 빠뜨려서 "회원가입은 되는데
  로그인은 안 되는" 버그가 있었다(아래 무료 티어 제약 참고).
...
4. 배포 중 `GEMINI_API_KEY`를 입력하라는 칸이 뜬다 (`sync: false`라 리포에는 안 들어가고
...
- ~~SQLite 파일이 재배포/재시작 시 초기화되는 문제~~ → `render.yaml`이 무료 Postgres
  (`unity-ai-assistant-db`)를 자동 프로비저닝하고 `DATABASE_URL`을 채워주도록 고쳤다(실제로
  이 문제 때문에 "회원가입은 되는데 기존 계정 로그인은 안 되는" 버그가 있었다 — 재배포마다
  SQLite 파일이 통째로 리셋되던 것). **다만 Render 무료 Postgres는 생성 후 90일 뒤 만료된다** —
  만료 임박 이메일이 오면 새 DB를 만들어 `DATABASE_URL`을 갱신해야 한다.
...
| Day 6 | **보류 (API 키 아직 미연결)** | GEMINI_API_KEY 연결 후 `scripts\run_scenario_eval.py`를 real 모드로 재실행해 실제 LLM 코드 생성 품질 확인, 부족하면 시나리오/스니펫 보강 |
